...
// ✅ อ้างจาก ENV เท่านั้น
const SITE_URL = BASE_URL;

// ให้ Next ดึงใหม่ทุกรอบ
export const dynamic = 'force-dynamic';

// ดึงโพสต์เดียวตาม slug
...
// ✅ Override metadata เฉพาะหน้าบทความ
export async function generateMetadata({ params: { slug, lang } }: Props): Promise<Metadata> {
  ...
  const url = `${SITE_URL}/${lang}/blog/${encodeURIComponent(post.slug)}`;
...
export default async function BlogPostPage({ params: { slug, lang } }: Props) {
  ...
  // ✅ ใช้ SITE_URL จริงใน JSON-LD
  const articleUrl = `${SITE_URL}/${lang}/blog/${encodeURIComponent(post.slug)}`;
  const articleSchema = {
            ...
            <article className="lg:col-span-3 prose lg:prose-xl max-w-none">
              ...
              <div className="mt-8 p-6 bg-emerald-50 rounded-lg border-l-4 border-emerald-500 not-prose">
                ...
                <p className="text-emerald-900">
                  {dict.blogPost.cta.description}{' '}
                  <Link href={`/${lang}/#services`} className="text-emerald-700 font-bold hover:underline">
                    {dict.blogPost.cta.link}
                  </Link>{' '}
                  {dict.blogPost.cta.end}
                </p>
              </div>
            </article>
                      ...
                      <Card key={related.id} className="overflow-hidden transition-shadow hover:shadow-md">
                        <Link href={relatedUrl} className="block">
                          ...
                          <CardContent className="p-4">
                            <h4 className="font-bold text/base leading-tight hover:text-emerald-600">
                              {related.title}
                            </h4>
                          </CardContent>
                        </Link>
